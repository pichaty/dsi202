...
1.  **Clone Repository:**
    ...
    ```
    **** แทนที่ `<URL_ของ_REPOSITORY_คุณ>` ด้วย URL จริงของ GitHub repository และ `<ชื่อ_ไดเรกทอรี_โปรเจกต์_pawpal>` ด้วยชื่อโฟลเดอร์ที่ถูกสร้างขึ้นหลังจากการ clone 


2.  **Build และ Run ด้วย Docker Compose :**
    รันคำสั่งต่อไปนี้เพื่อ build Docker images และ start containers:
    ```bash
