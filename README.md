...
1.  **Clone Repository:**
    ...
    ```
    **หมายเหตุ:** แทนที่ `<URL_ของ_REPOSITORY_คุณ>` ด้วย URL จริงของ GitHub repository และ `<ชื่อ_ไดเรกทอรี_โปรเจกต์_pawpal>` ด้วยชื่อโฟลเดอร์ที่ถูกสร้างขึ้นหลังจากการ clone (โดยทั่วไปคือ `pawpal` หาก clone มาจาก repository ชื่อ `pawpal`)
...
3.  **Build และ Run ด้วย Docker Compose:**
    ...
    ```
    คำสั่งนี้อาจใช้เวลาสักครู่ในครั้งแรกที่รัน เนื่องจากต้องดาวน์โหลดและ build images ต่างๆ รอจนกระทั่งเห็น log ว่า web server เริ่มทำงาน
